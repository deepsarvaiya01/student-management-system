...
public class CourseAnalysisDAO {
	...
	public Map<String, Integer> getMostEnrolledCourses() throws SQLException {
		...
		String sql = "SELECT c.course_name, COUNT(sc.student_id) as enrollment_count " + "FROM courses c "
				+ "LEFT JOIN student_courses sc ON c.course_id = sc.course_id AND sc.is_active = 1 "
				+ "WHERE c.is_active = 1 " + "GROUP BY c.course_id, c.course_name " + "ORDER BY enrollment_count DESC";
	...
	public Map<String, Integer> getCourseWiseStudentCount() throws SQLException {
		...
		String sql = "SELECT c.course_name, COUNT(DISTINCT sc.student_id) as student_count " + "FROM courses c "
				+ "LEFT JOIN student_courses sc ON c.course_id = sc.course_id AND sc.is_active = 1 "
				+ "WHERE c.is_active = 1 " + "GROUP BY c.course_id, c.course_name " + "ORDER BY student_count DESC";
	...
	public Map<String, Map<String, Integer>> getSubjectDistributionPerCourse() throws SQLException {
		...
		String sql = "SELECT c.course_name, s.subject_name, s.subject_type, COUNT(*) as subject_count "
				+ "FROM courses c " + "JOIN subject_course sc ON c.course_id = sc.course_id "
				+ "JOIN subjects s ON sc.subject_id = s.subject_id " + "WHERE c.is_active = 1 AND s.is_active = 1 "
	...
	public Map<String, Map<String, Object>> getCourseCompletionStatus() throws SQLException {
		Map<String, Map<String, Object>> completionStatus = new LinkedHashMap<>();

		String sql = "SELECT c.course_name, " + "COUNT(sc.student_id) as total_enrolled, "
				+ "SUM(CASE WHEN f.pending_amount = 0 THEN 1 ELSE 0 END) as completed, "
				+ "SUM(CASE WHEN f.pending_amount > 0 THEN 1 ELSE 0 END) as in_progress " + "FROM courses c "
				+ "LEFT JOIN student_courses sc ON c.course_id = sc.course_id AND sc.is_active = 1 "
				+ "LEFT JOIN fees f ON sc.student_course_id = f.student_course_id " + "WHERE c.is_active = 1 "
				+ "GROUP BY c.course_id, c.course_name " + "ORDER BY total_enrolled DESC";
		...
		try (PreparedStatement stmt = connection.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
			while (rs.next()) {
				...
				status.put("total_enrolled", totalEnrolled);
				status.put("completed", completed);
				status.put("in_progress", inProgress);
	...
	public Map<String, Integer> getTeacherWorkload() throws SQLException {
		...
		String sql = "SELECT t.name, COUNT(st.subject_id) as subject_count " + "FROM teachers t "
				+ "LEFT JOIN subject_teachers st ON t.teacher_id = st.teacher_id " + "WHERE t.is_active = 1 "
				+ "GROUP BY t.teacher_id, t.name " + "ORDER BY subject_count DESC";
	...
	public List<String> getTeachersWithMaxSubjects() throws SQLException {
		...
		String sql = "SELECT t.name, COUNT(st.subject_id) as subject_count " + "FROM teachers t "
				+ "LEFT JOIN subject_teachers st ON t.teacher_id = st.teacher_id " + "WHERE t.is_active = 1 "
				+ "GROUP BY t.teacher_id, t.name " + "HAVING COUNT(st.subject_id) >= 3 "
	...
	public List<String> getAvailableTeachersForAssignment() throws SQLException {
		...
		String sql = "SELECT t.name, COUNT(st.subject_id) as subject_count " + "FROM teachers t "
				+ "LEFT JOIN subject_teachers st ON t.teacher_id = st.teacher_id " + "WHERE t.is_active = 1 "
				+ "GROUP BY t.teacher_id, t.name " + "HAVING COUNT(st.subject_id) < 3 " + "ORDER BY subject_count ASC";
	...
	public Map<String, List<String>> getTeacherSubjectDistribution() throws SQLException {
		...
		String sql = "SELECT t.name, s.subject_name, s.subject_type " + "FROM teachers t "
				+ "JOIN subject_teachers st ON t.teacher_id = st.teacher_id "
				+ "JOIN subjects s ON st.subject_id = s.subject_id " + "WHERE t.is_active = 1 AND s.is_active = 1 "
	...
	public Map<String, Integer> getMandatoryVsElectiveSubjects() throws SQLException {
		...
		String sql = "SELECT subject_type, COUNT(*) as count " + "FROM subjects " + "WHERE is_active = 1 "
	...
	public Map<String, Integer> getSubjectPopularity() throws SQLException {
		...
		String sql = "SELECT s.subject_name, COUNT(sc.student_id) as enrollment_count " + "FROM subjects s "
				...
				+ "WHERE s.is_active = 1 " + "GROUP BY s.subject_id, s.subject_name "
	...
	public Map<String, Map<String, Object>> getCourseFeeAnalysis() throws SQLException {
		Map<String, Map<String, Object>> feeAnalysis = new LinkedHashMap<>();

		String sql = "SELECT c.course_name, c.total_fee, AVG(f.paid_amount) as avg_paid, "
				+ "MIN(f.total_fee) as min_fee, MAX(f.total_fee) as max_fee " + "FROM courses c "
				+ "LEFT JOIN student_courses sc ON c.course_id = sc.course_id AND sc.is_active = 1 "
				+ "LEFT JOIN fees f ON sc.student_course_id = f.student_course_id " + "WHERE c.is_active = 1 "
				+ "GROUP BY c.course_id, c.course_name, c.total_fee " + "ORDER BY c.total_fee DESC";
	...
	public Map<String, Integer> getSemesterWiseStructure() throws SQLException {
		...
		String sql = "SELECT c.course_name, c.no_of_semester, COUNT(sc.student_id) as student_count "
				+ "FROM courses c " + "LEFT JOIN student_courses sc ON c.course_id = sc.course_id AND sc.is_active = 1 "
				+ "WHERE c.is_active = 1 " + "GROUP BY c.course_id, c.course_name, c.no_of_semester "
